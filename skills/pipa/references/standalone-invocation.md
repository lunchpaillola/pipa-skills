...
| `reach`, `find-threads` | `experimental-pipa-reach` | User wants finished work to find relevant public conversations, questions, threads, communities, or places to contribute helpfully | Experimental scope, source privacy, verified thread URLs, manual-only community participation, no invented search results |
...
- `write a blog post`, `create a marketing plan`, `run paid ads`, and `build a social calendar` do not route to `experimental-pipa-reach` unless the user specifically asks for thread/community discovery around existing work.
- `post this everywhere now` may route to `experimental-pipa-reach` only for discovery guidance; Reach must not post, submit, comment, vote, schedule, or create drafts by default.
